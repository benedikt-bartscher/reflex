...
def UploadFile():
    ...
    def index():
        return rx.vstack(
            ...
            rx.button(
                "Upload",
                on_click=lambda: UploadState.handle_upload(rx.upload_files()),  # type: ignore [reportArgumentType]
                id="upload_button",
            ),
            rx.box(
                rx.foreach(
...
async def test_upload_file(
    ...
    if secondary:
        ...
        assert state.substates[state_name].event_order.count("upload_progress") == 1  # type: ignore[reportAttributeAccessIssue]
        assert state.substates[state_name].event_order.count("chain_event") == 1  # type: ignore[reportAttributeAccessIssue]
    ...
    async def get_file_data():
        return (
            ...
            ._file_data  # type: ignore[reportAttributeAccessIssue]
        )
...
async def test_cancel_upload(tmp_path, upload_file: AppHarness, driver: WebDriver):
    ...
    async def _progress_dicts():
        state = await upload_file.get_state(substate_token)
        return state.substates[state_name].progress_dicts  # type: ignore[reportAttributeAccessIssue]
    ...
    state = await upload_file.get_state(substate_token)
    file_data = state.substates[state_name]._file_data  # type: ignore[reportAttributeAccessIssue]
    assert isinstance(file_data, dict)
    normalized_file_data = {Path(k).name: v for k, v in file_data.items()}
...
async def test_on_drop(
    ...
    async def exp_name_in_quaternary():
        state = await upload_file.get_state(substate_token)
        return exp_name in state.substates[state_name].quaternary_names  # type: ignore[reportAttributeAccessIssue]
    ...
    state = await upload_file.get_state(substate_token)
    assert exp_name in state.substates[state_name].quaternary_names  # type: ignore[reportAttributeAccessIssue]
