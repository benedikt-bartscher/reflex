...
async def test_computed_vars(
    ...
    assert state.count1_backend == 0  # type: ignore[reportAttributeAccessIssue]
    assert state._count1_backend == 0  # type: ignore[reportAttributeAccessIssue]
    ...
    state = (await computed_vars.get_state(token)).substates[state_name]
    assert state is not None
    assert state.count1_backend == 1  # type: ignore[reportAttributeAccessIssue]
    assert count1_backend.text == ""
    assert state._count1_backend == 1  # type: ignore[reportAttributeAccessIssue]
    assert count1_backend_.text == ""
