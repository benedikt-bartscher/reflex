...
async def test_event_chain_click(
    ...
    async def _has_all_events():
        return len(
            (await event_chain.get_state(token)).substates[state_name].event_order  # type: ignore[reportAttributeAccessIssue]
        ) == len(exp_event_order)

    await AppHarness._poll_for_async(_has_all_events)
    event_order = (await event_chain.get_state(token)).substates[state_name].event_order  # type: ignore[reportAttributeAccessIssue]
...
async def test_event_chain_on_load(
    ...
    assert backend_state.event_order == exp_event_order  # type: ignore[reportAttributeAccessIssue]
    assert backend_state.is_hydrated is True  # type: ignore[reportAttributeAccessIssue]
