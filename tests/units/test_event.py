...
def test_event_actions_on_state():
    ...
    sp_handler = EventActionState.handler.stop_propagation  # type: ignore [reportFunctionMemberAccess]
    assert sp_handler.event_actions == {"stopPropagation": True}
...
def test_event_decorator_with_event_actions():
    ...
    assert hasattr(bg_temporal_handler.fn, BACKGROUND_TASK_MARKER)  # type: ignore [reportAttributeAccessIssue]
...
def test_event_decorator_backward_compatibility():
    ...
    assert not hasattr(old_handler.fn, BACKGROUND_TASK_MARKER)  # type: ignore [reportAttributeAccessIssue]
    ...
    assert hasattr(bg_handler.fn, BACKGROUND_TASK_MARKER)  # type: ignore [reportAttributeAccessIssue]
