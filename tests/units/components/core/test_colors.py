...
color_with_fstring = rx.color(
    f"{ColorState.color}",  # type: ignore [reportArgumentType]
    ColorState.shade,
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            create_color_var(
                rx.color(
                    f"{ColorState.color_part}ato",  # type: ignore [reportArgumentType]
                    ColorState.shade,
                )
            ),
...
def test_radix_color(color, expected):
    ...
    assert str(code_block.custom_style["backgroundColor"]) == expected  # type: ignore [reportAttributeAccessIssue]
