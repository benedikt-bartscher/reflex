        ...
        pytest.param(
            {
                "data": pd.DataFrame(
                    [["foo", "bar"], ["foo1", "bar1"]],
                    columns=["column1", "column2"],  # type: ignore [reportArgumentType]
                )
            },
            "data",
        ),
...
def test_validate_data_table(data_table_state: rx.State, expected):
    ...
    """
    if not types.is_dataframe(data_table_state.data._var_type):  # type: ignore[reportAttributeAccessIssue]
        data_table_component = DataTable.create(
            data=data_table_state.data,  # type: ignore[reportAttributeAccessIssue]
            columns=data_table_state.columns,  # type: ignore[reportAttributeAccessIssue]
        )
    else:
        data_table_component = DataTable.create(data=data_table_state.data)  # type: ignore[reportAttributeAccessIssue]
